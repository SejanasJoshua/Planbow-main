...
import { Validate } from '@data/IdeationAttributes';
import { v4 as uuidv4 } from 'uuid';
import PropTypes from 'prop-types';

export default function IdeaValidate({ attributes, setAttributes, saveData }) {
	const [attributesData, setAttributesData] = useState(Validate);
	...
	useEffect(() => {
		...
		setAttributes({
			...attributes,
			validate: attributesData,
		});
	}, [attributesData]);
	useEffect(() => {
		attributes?.validate && setAttributesData(attributes.validate);
	}, []);
